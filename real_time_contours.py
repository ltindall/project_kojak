...
base_path = '~/epark'
#model = load_model(base_path+'/project_kojak/models/VGG_cross_validated.h5')
model = load_model('/Users/ltindall/epark/project_kojak/models/VGG_cross_validated.h5')
...
def predict_rgb_image_vgg(image):
    ...
    print(f'pred_array: {pred_array}', flush=True)
    result = gesture_names[np.argmax(pred_array)]
    print(f'Result: {result}', flush=True)
    print(max(pred_array[0]), flush=True)
...
while camera.isOpened():
    ...
    if isBgCaptured == 1:
        ...
        contours, hierarchy = cv2.findContours(thresh1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1)
        length = len(contours)
        maxArea = -1
        if length > 0:
            ...
            res = np.squeeze(res).astype(float) # squeeze to remove empty dimension and convert to float
            res[:,0] = res[:,0]/float(drawing.shape[1]) # normalize x dimension
            res[:,1] = res[:,1]/float(drawing.shape[0]) # normalize y dimension
            # values should now be bounded (0,1)
            res = (res - 0.5) # center around 0, values are now (-0.5, 0.5)
            res = res * 2 # scale values to be (-1,1)
            res[:,1] *= -1 # flip y values because opencv flips y direction


            for x,y in res:
                print('{:.4f},{:.4f}'.format(x,y))

            #print('x max = {}, x min = {}'.format(np.max(res[:,0]), np.min(res[:,0])))
            #print('y max = {}, y min = {}'.format(np.max(res[:,1]), np.min(res[:,1])))

            #print('output shape = ',drawing.shape)
            #print('countour shape = ',res.shape)
    ...
    if k == 27:  # press ESC to exit all windows at any time
        ...
        time.sleep(2)
        isBgCaptured = 1
        print('Background captured', flush=True)
        #pygame.init()
        #pygame.mixer.init()
        #pygame.mixer.music.load(base_path+'/1-05 Virtual Insanity.mp3')
